...
public class ResultsConverterImpl_UnitTests
{
	...
	public void getImageFromResults_WithMocks_ExpectImage() throws InterruptedException, ExecutionException
	{
		...
		BaseMatcher<int[][][]> matchesCombinedArray = ResultsConvertImpl_TestsHelper.getMatcher(combinedArray);
		...
		Mockito.verify(mockScaler, Mockito.times(1)).scale(Mockito.argThat(matchesCombinedArray));
		...
		Assert.assertEquals(mockImage, retImage);
	}
}
