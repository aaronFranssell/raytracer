...
public class TestsHelper
{
	public static void arrayListSubsets(ArrayList<?> expected, ArrayList<?> actual)
	{
		...
		for(int i = 0; i < expected.size(); i++)
		{
			...
			if(!found)
			{
				Assert.fail("Did not find: " + expected.get(i) + " in actual list.");
			}
		}
		...
		for(int i = 0; i < actual.size(); i++)
		{
			...
			if(!found)
			{
				Assert.fail("Did not find: " + actual.get(i) + " in expected list.");
			}
		}
	}
	...
	public static void doubleArraysSubsets(double[] expected, double[] actual)
	{
		...
		for(int i = 0; i < expected.length; i++)
		{
			...
			if(!found)
			{
				Assert.fail("Did not find: " + expected[i] + " in actual list.");
			}
		}
		...
		for(int i = 0; i < actual.length; i++)
		{
			...
			if(!found)
			{
				Assert.fail("Did not find: " + actual[i] + " in expected list.");
			}
		}
	}
