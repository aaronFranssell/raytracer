...
public class Cone extends Surface
{
	...
	public Cone(Vector incomingDirection, Point incomingVertex, double incomingAlpha, Point incomingBasePoint, double incomingLength, 
				Color incomingCR, Color incomingCA, Color incomingCL, Effects incomingEffects, Util incomingOps) throws RaytracerException
	{
		     ...
		     Library.doubleEqual(vertex.z, basePoint.z, Constants.POSITIVE_ZERO)))
		{
			...
			{
				throw new RaytracerException("The base point: " + basePoint.toString() + " is not on the same line as the vertex: " 
	...
	public Cone(Vector incomingDirection, Point incomingVertex, double incomingAlpha, Point incomingBasePoint, double incomingLength, 
			Color incomingCR, Color incomingCA, Color incomingCL, Effects incomingEffects) throws RaytracerException
	{
	 ...
	 * @throws Exception 
	 */
	private ArrayList<Double> getHitTsByLimitingLength(double[] incomingHitTs, Ray r) throws RaytracerException
	{
		...
		for(int i = 0; i < incomingHitTs.length; i++)
		{
			...
			if(direction.dot(vectorToHitPointFromBaseNormalized) > 0)
			{
				...
				*/
				double solidDirectionLength = direction.dot(vectorToHitPointFromBaseNormalized) * vectorToHitPointFromBase.magnitude();
