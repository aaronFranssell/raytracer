...
public class NightSkyBackground extends NoiseColor
{
	...
	private Color getCloudColor(Point p)
	{
		...
		double val = 0.0;
		val += FastMath.abs(perlinNoise(point, 128, 0.45));
		val += FastMath.abs(perlinNoise(point, 128, 0.7))/2;
		val += FastMath.abs(perlinNoise(point, 128, 0.9))/4;
		if(val < 0.2)
		{
			return new Color(0.0,0.0,0.0);
		}
		val += 0.1;
		val = FastMath.min(val, 1.0);
