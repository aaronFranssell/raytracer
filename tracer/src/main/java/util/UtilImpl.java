...
public class UtilImpl implements Util {
	...
	public Ray getReflectedRay(Ray r, Point p, Vector n) {
		/* V is the ray direction, N is the surface normal c1 = -dot_product( N, V ) Rl = V + (2 * N * c1 ) */
	...
	public boolean isInShadow(Scene scene, Point light, HitData hitData) throws RaytracerException {
		...
		if (shadowData.isHit()
				&& shadowData.getSurface().getType() != Surface.SurfaceType.Outersphere) {
			// if the light point occurs closer to a hit point than this object, then the object is not in
			// shadow
	...
	public Color getColorLambertian(
		...
		if (inShadow) {
			...
			RGBValue.red = cR.red * (cA.red + cL.red * Math.max(0, nDotL));
			RGBValue.green = cR.green * (cA.green + cL.green * Math.max(0, nDotL));
			RGBValue.blue = cR.blue * (cA.blue + cL.blue * Math.max(0, nDotL));
		}
		return RGBValue;
	}
	...
	public Color getColorPhong(
		...
		double phongHighlight = Math.max(0, n.dot(h));
	...
	public double[] solveQuartic(double A, double B, double C, double D, double E) {
		...
		double gamma = -3 * Math.pow(B, 4) / (256 * Math.pow(A, 4))
				+ C * Math.pow(B, 2) / (16 * Math.pow(A, 3))
				- B * D / (4 * Math.pow(A, 2))
				+ E / A;
		...
		Complex Q = new Complex(
				((-1) * Math.pow(alpha, 3) / 108) + (alpha * gamma / 3) - (Math.pow(beta, 2) / 8), 0.0);
		Complex R;
		...
		// if U == 0...
		if (UtilImpl.doubleEqual(U.getReal(), 0.0, Constants.POSITIVE_ZERO)
				&& UtilImpl.doubleEqual(U.getImaginary(), 0.0, Constants.POSITIVE_ZERO)) {
	...
	public int[] getCircleUVImageMapping(Point p, Point center, double radius, int w, int h) {
		double theta = Math.acos((p.y - center.y) / radius);
		double phi = Math.atan2(p.z - center.z, p.x - center.x);
		if (phi < 0.0) {
			phi += 2 * Math.PI;
		}
